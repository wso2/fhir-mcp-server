...
class TestCreateAsyncFhirClient:
    ...
    async def test_create_client_with_custom_timeout(self):
        ...
        config = ServerConfigs(
            server_base_url="https://example.fhir.org/R4", mcp_request_timeout=60
        )

        with (
            patch("fhir_mcp_server.utils.AsyncFHIRClient") as mock_client,
...
class TestGetCapabilityStatement:
    ...
    async def test_get_capability_statement_success(self):
        ...
        with patch(
            "fhir_mcp_server.utils.create_mcp_http_client"
            ...
            mock_client.get.assert_called_once_with(
                url=metadata_url, headers=get_default_headers()
            )
